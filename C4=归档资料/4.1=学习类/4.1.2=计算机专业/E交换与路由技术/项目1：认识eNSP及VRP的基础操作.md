...
*   **领会**：熟悉VRP的命令和基本操作。
*   **应用**：能正确安装eNSP模拟器；能使用eNSP搭建和配置网络；能掌握VRP平台的应用；能熟悉VRP基本操作。
...
#### 3.0 VRP的来源与发展 (识记内容)
*   **来源**：VRP（Versatile Routing Platform）是华为公司数据通信产品的通用操作系统平台。它随着华为数通产品的发展而演进，最初是基于简单的网络功能构建的。
*   **发展历程**：
    *   **VRP1.0/2.0**：早期版本，功能较为基础，主要支持简单的静态路由和基础协议。
    *   **VRP3.0/5.0**：进入快速发展期，实现了分布式架构，支持大规模组网、多业务融合及高可靠性。目前大多数实验环境和现网设备运行在 VRP5 版本。
    *   **VRP8.0**：最新一代版本，采用全业务分布式架构，支持容器化、云化及更强的自动化运维能力，适用于超大规模数据中心和运营商核心网。
...
#### 步骤3.5：配置PC端的IP（如果连了PC）
路由器配好了，如果它连接了一台电脑（PC1），电脑也要配IP才能通。
1.  **双击**画布上的 PC1 图标。
2.  在弹出的窗口中，找到 **"基础配置" (Basic Config)** 选项卡。
3.  **IPv4配置**：
    *   **IP地址**：填 `192.168.1.2` (要在同一个网段，即前三段数字一样)。
    *   **子网掩码**：填 `255.255.255.0`。
    *   **网关 (Gateway)**：填刚才给路由器接口配的 IP `192.168.1.1` (路由器的接口就是电脑通往外界的大门)。
4.  点击右下角的 **"应用" (Apply)**。
