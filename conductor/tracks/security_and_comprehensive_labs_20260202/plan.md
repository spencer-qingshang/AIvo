# Track: 补充项目6、7大纲及生成园区网实操手册 (plan.md)

## 1. 计划概述
本计划将分阶段完成项目6和项目7的理论补充及4份实操手册的编写。

---

## 第一阶段：理论大纲深度补充 (Theory Update) [checkpoint: 3d6429b]

- [x] Task: 补充“项目6 构建安全的园区网络.md”理论内容 3d6429b
- [x] Task: 补充“项目7 园区网络综合实训.md”理论内容 3d6429b
- [x] Task: Conductor - User Manual Verification '第一阶段：理论大纲深度补充' (Protocol in workflow.md)

---

## 第二阶段：项目6 实操手册编写 (Security Manuals) [checkpoint: a0e2627]

- [x] Task: 编写“项目6_1_接入安全实战.md” a0e2627
- [x] Task: 编写“项目6_2_ACL与NAT实战.md” a0e2627
- [x] Task: 编写“项目6_3_防火墙基础实战.md” a0e2627
- [x] Task: Conductor - User Manual Verification '第二阶段：项目6 实操手册编写' (Protocol in workflow.md)

---

## 第三阶段：项目7 综合实训手册编写 (Final Lab) [checkpoint: ae997da]

- [x] Task: 编写“项目7_综合组网演练手册.md” 644f40b
- [x] Task: Conductor - User Manual Verification '第三阶段：项目7 综合实训手册编写' (Protocol in workflow.md)

---

## 修正阶段：补充详细物理连接步骤 (Revision)

- [x] Task: 修改“项目6_1_接入安全实战.md”增加设备选型与连线细节
- [x] Task: 修改“项目6_2_ACL与NAT实战.md”增加设备选型与连线细节
- [x] Task: 修改“项目6_3_防火墙基础实战.md”增加设备选型与连线细节
- [~] Task: 修改“项目7_综合组网演练手册.md”增加设备选型与连线细节

---

## 第四阶段：归档与登记 (Finalization)

- [x] Task: 更新 Tracks Registry
- [x] Task: 更新 Metadata
- [x] Task: Conductor - User Manual Verification '第四阶段：归档与登记' (Protocol in workflow.md)
