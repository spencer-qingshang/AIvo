...
### 7.1 识记
- **RIP (Routing Information Protocol)：** 路由信息协议，一种基于**距离矢量算法**的动态路由协议。
- **RIP 版本：** 
  - **v1:** 有类别路由协议，广播更新，不支持 VLSM。
  - **v2:** 无类别路由协议，组播更新（224.0.0.9），支持 VLSM 和认证。
- **RIP 定时器：**
  - 更新定时器（Update）：30s。
  - 老化定时器（Age）：180s。
  - 垃圾收集定时器（Garbage-collect）：120s。

### 7.2 领会
- **RIP 的原理 and 作用：**
  - **原理：** 以跳数（Hop Count）作为度量值，最大跳数为 15，16 跳视为不可达。
  - **作用：** 适用于小型同构网络。
- **RIPv1 与 RIPv2 区别：** v2 支持掩码信息、支持组播、安全性更高。

### 7.3 应用
- **RIPv2 配置：**
```bash
[Router1] rip 1                                     # 启动进程 1
[Router1-rip-1] version 2                           # 指定版本 2
[Router1-rip-1] network 192.168.1.0                 # 宣告直连网段 (必须是自然网段)
...
### 8.1 识记
- **OSPF (Open Shortest Path First)：** 开放最短路径优先协议，一种基于**链路状态算法**的动态路由协议。
- **OSPF 协议区域：** 
  - **Area 0:** 骨干区域（Backbone Area），所有非骨干区域必须与骨干区域相连。
  - **非骨干区域:** 用于扩展网络。

### 8.2 领会
- **OSPF 协议的原理 and 作用：**
  - **原理：** 路由器交换链路状态信息（LSA），构建全网拓扑图（LSDB），使用 SPF 算法计算最短路径。
  - **作用：** 收敛快，无环路，适用于大中型网络。
- **骨干区域与非骨干区域的区别：** 骨干区域是中枢，负责跨区域的路由转发；非骨干区域负责本区域内的路由。

### 8.3 应用
- **OSPF 单区域配置：**
```bash
[Router1] ospf 1 router-id 1.1.1.1                  # 启动进程并设置 Router ID
[Router1-ospf-1] area 0                             # 进入骨干区域
[Router1-ospf-1-area-0.0.0.0] network 192.168.1.0 0.0.0.255 # 宣告网段 (反掩码)
...
### 2. 填空题
**题目**：局域网中常用的路由协议有 \_\_\_、 \_\_\_、 \_\_\_ 三种。

**参考答案**：RIP、OSPF、IS-IS/BGP（注：根据大纲，RIP 与 OSPF 为重点）

### 3. 综合应用题：OSPF 动态路由配置
**题目**：根据拓扑图配置动态 OSPF 路由协议，实现全网互通。要求完成路由器端口 IP 配置、PC IP/网关分配以及 OSPF 协议配置。

**配置参考（笔试答题模板）**：
*   **第一步：配置各设备端口 IP (以 R1 为例)**
    ```shell
    [R1] interface G0/0/0
    [R1-G0/0/0] ip address 192.168.1.254 24
    [R1] interface S1/0/0
    [R1-S1/0/0] ip address 10.1.1.1 24
    ```
*   **第二步：配置 PC 网关**
    *(注：在笔试中记录规划结果，如 PC1: 192.168.1.1, GW: 192.168.1.254)*
*   **第三步：配置 OSPF 协议 (以 R1 为例)**
    ```shell
...
### 10.1 核心理论必背
- **路由表的五要素**：目的网段、子网掩码、出接口、下一跳、优先级。
- **RIP 协议限制**：最大跳数为 15，16 跳不可达。
- **OSPF 邻居建立条件**：Area ID 相同、Hello 时间一致、网段掩码匹配等。

### 10.2 必背命令关键字
- `ip route-static` (静态路由)。
- `rip` / `version 2` / `network` (RIP 宣告)。
- `ospf` / `area` / `network` (OSPF 宣告，注意使用反掩码)。
- `display ip routing-table` (查看核心表项)。
- `display ip peer/neighbor` (查看 OSPF 状态)。

### 10.3 常见排错思路
- **拼不通 (Ping Failure)**：
  1. 查本地接口 `display ip interface brief`。
  2. 查路由表 `display ip routing-table` (是否有回程路由?)。
  3. 查 PC 网关配置。
