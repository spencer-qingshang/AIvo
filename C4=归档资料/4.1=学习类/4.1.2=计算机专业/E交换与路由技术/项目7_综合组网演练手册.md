...
#### 3.1 配置 PC (DHCP 模式)
我们希望终端自动获取 IP，所以配置很简单：
1.  **双击 PC1**：
    *   在“基础配置”页签，选择 **DHCP**。
    *   点击 **应用**。
2.  **双击 PC2**：
    *   同上，选择 **DHCP** 并应用。
    *   *注意*：此时还获取不到 IP，因为 DHCP 服务器 (CoreSW) 还没配。
...
[ISP-GigabitEthernet0/0/0] ip address 202.100.1.1 24
[ISP-GigabitEthernet0/0/0] quit

# 配置连接服务器的接口
[ISP] interface g0/0/1
[ISP-GigabitEthernet0/0/1] ip address 202.100.1.2 24
[ISP-GigabitEthernet0/0/1] quit
```

### 4. 网络规划总览 (心中有数)
虽然我们有了详细步骤，但这张表可以帮你理清思路：
| 区域 | VLAN | IP网段 | 网关 (CoreSW) |
| :--- | :--- | :--- | :--- |
| **员工区** | VLAN 10 | 192.168.10.0/24 | 192.168.10.254 |
| **无线区** | VLAN 20 | 192.168.20.0/24 | 192.168.20.254 |
| **管理区** | VLAN 100 | 192.168.100.0/24| 192.168.100.254|
| **互联区** | VLAN 800 | 10.1.1.0/24 | 10.1.1.1 |
