...
### 3. 初始化配置 (路由器模拟 PC)
我们用两台路由器来扮演 PC 和服务器，这样环境更稳定。
> **注意**：启动后，双击设备进入命令行配置。

*   **配置内网 PC (PC1_Sim)**:
    ```shell
    <Huawei> system-view
    [Huawei] sysname PC1_Sim
    
    # 配置接口 IP
    [PC1_Sim] interface g0/0/0
    [PC1_Sim-GigabitEthernet0/0/0] ip address 192.168.1.10 24
    [PC1_Sim-GigabitEthernet0/0/0] quit
    
    # 配置默认网关 (指向防火墙内网口)
    [PC1_Sim] ip route-static 0.0.0.0 0 192.168.1.1
    ```

*   **配置外网服务器 (Server_Sim)**:
    ```shell
    <Huawei> system-view
    [Huawei] sysname Server_Sim
    
    # 配置接口 IP
    [Server_Sim] interface g0/0/0
    [Server_Sim-GigabitEthernet0/0/0] ip address 202.100.1.2 24
    [Server_Sim-GigabitEthernet0/0/0] quit
    
    # 配置默认网关 (指向防火墙外网口)
    [Server_Sim] ip route-static 0.0.0.0 0 202.100.1.1
    ```
