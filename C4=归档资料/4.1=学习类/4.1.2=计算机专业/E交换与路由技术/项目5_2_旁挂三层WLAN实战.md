...
### 1. 拖动设备
*   **AC (无线控制器)**：拖入 1 台 **AC6005**。
*   **核心交换机 (Core)**：拖入 1 台 **S5700**，命名为 `CoreSW`。
*   **接入交换机 (Access)**：拖入 1 台 **S3700**，命名为 `AccSW`。
*   **AP**: 拖入 1 台 **AP6050**。
*   **STA**: 拖入 1 台 **STA** (无线终端)。

### 2. 物理连线 (严格按图连)
*   **CoreSW** 的 `GE 0/0/1` <--> 连 **AC** 的 `GE 0/0/1`。
*   **CoreSW** 的 `GE 0/0/2` <--> 连 **AccSW** 的 `GE 0/0/1`。
*   **AccSW** 的 `GE 0/0/2` <--> 连 **AP** 的 `GE 0/0/0`。
